defmodule MyEnumTest do
  ...
  test "drop_while" do
    assert MyEnum.drop_while([1, 2, 3, 2, 1], fn x -> x < 3 end) == [3, 2, 1]
    assert MyEnum.drop_while([1, 2, 3, 2, 1], fn x -> x < 2 end) == [2, 3, 2, 1]
  end
  ...
  test "sort/1" do
    assert MyEnum.sort([3, 2, 1]) == [1, 2, 3]
  end

  test "sort/2" do
    assert MyEnum.sort([1, 2, 3], &(&1 >= &2)) == [3, 2, 1]
    assert MyEnum.sort(["some", "kind", "of", "monster"], &(byte_size(&1) <= byte_size(&2))) == ["of", "some", "kind", "monster"]
